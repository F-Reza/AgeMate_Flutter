...
class AgeController {
  ...
  static DateDuration timeToNextBirthday(DateTime birthdate,{DateTime? fromDate}) {
    ...
    DateTime nextBirthdayDate = tempDate.isBefore(endDate) 
        ? AgeController.add(date: tempDate, duration: DateDuration(years: 1))
        : tempDate;
 ...
 static String findDayName() {
    ...
    var sp = selectedBrithDate.toString().split(' ')[0].split("-");
...
DateTime selectedBrithDate = DateTime.now().subtract(const Duration(days: 1));
